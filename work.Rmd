...
## Problème des XY au centre de Paris
...
dev.off()
plot(1:nrow(out), out$x, log="y",pch=20, cex=0.4, col="red")
head(out)
x <- st_as_sf(as.data.frame(out), coords = c("lng","lat"), crs = 3035)
...
library(cartography)
opar <- par(mar=c(0,0,0,0))
plot(st_geometry(my_grid), col="grey", border="white",lwd=0.2)
plot(river, add=T, col = "#AAD3DF", border="#AAD3DF", lwd= 0.5)
plot(parc, add=T, col = "#AACBAF", border=NA)
propSymbolsLayer(x = x[50000:nrow(x),],var="x", inches=0.1)
labelLayer(x = x[2:50,], txt = "id", pos=2, cex=1, halo = TRUE, overlap = F )
...
nuser <- aggregate(xx$id,by=list(u=xx$from_user_id),length)
nuser <- nuser[order(nuser$x, decreasing = T),]
nuser <- nuser[nuser$x>=36,]

dim(nuser)
k <- 15
my_ts <- aggregate(tw_geo$id[tw_geo$from_user_id==nuser[k,"u"]],
                   by = list(date=substr(tw_geo$fulldate[tw_geo$from_user_id==nuser[k,"u"]],1,10)), 
                   length)
calendarHeat(my_ts$date, my_ts$x,ncolor=5, col = cartography::carto.pal(pal1 =  "turquoise.pal",20))
...
for(i in 1:20){
  plot(st_geometry(my_grid), col="black", border=NA,lwd=0.1)
  plot(river, add=T, col = "#AAD3DF", border="#AAD3DF", lwd= 0.5)
  plot(parc, add=T, col = "#AACBAF", border=NA)
  plot(st_geometry(xx[xx$from_user_id %in% nuser$u[i],]),cex = 1, pch = 20, col = "red", add=T)
}
...
plot(river, add=T, col = "#AAD3DF", border="#AAD3DF", lwd= 0.5)
plot(parc, add=T, col = "#AACBAF", border=NA)
plot(xx$geometry, cex = 0.1, pch=20, col='orange', add=T)
...
toto <- aggregate(xx$id,by=list(u=xx$from_user_id),length)
my_ts <- aggregate(xx$id,by = list(date=substr(xx$fulldate,1,10)), length)
print(sum(my_ts$x))
print(nrow(toto))
...
my_ts <- aggregate(t$id,by = list(date=substr(tw_geo$fulldate,1,10)), length)

head(my_ts)
calendarHeat(my_ts$date, my_ts$x,ncolor=12, col = cartography::carto.pal(pal1 = "wine.pal", 12))
