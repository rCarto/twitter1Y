...
tw_geo$fulldate <- lubridate::with_tz(as.POSIXct(tw_geo$created_at), 
                                      "Europe/Paris")
...
**On peut commencer ici si on veut**
...
```{r, cache=TRUE}
tw_geo <- tw_geo[tw_geo$source %in% client[client$nuser >= 10 & client$tweetpuser <= 50, "source"],]
```
...
```{r, eval=FALSE}
library(sf)
# aggrégation des tweets par couple de coordonnées ("points de tweets")
coords <- st_coordinates(tw_geo)
xx <- cbind(tw_geo, coords)
twagcoord <- aggregate(xx$id, 
                       by = list(lng=coords[,1], lat=coords[,2]), 
                       FUN = length)
out <- twagcoord[order(twagcoord$x, decreasing = T),] 

# transformation des points de tweets en objets spatiaux
out <- out[out$x>=10,]
...
plot(x[c(5,102,20,109),"geometry"])
c(5,102,20,109)
c(8,13)
c(14,188)
...
library(cartography)
osm <- getTiles(x[c(5,102,20,109),], zoom=16)
tilesLayer(osm)
plot(x[c(5,102,20,109),"geometry"], add=T)
out[c(5,102,20,109),]
plot(tw_geo$geometry,add=T, col="red", cex = 0.2)
